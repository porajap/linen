...
function CreateDocument($conn, $DATA)
{
  ...
  function SelectDocument($conn, $DATA)
  {
    ...
    $DocNo = $DATA["xdocno"];
    $Datepicker = $DATA["Datepicker"];
    $Sql = "SELECT   site.HptName,department.DepName,repair.DocNo,repair.DocDate,repair.Total,users.FName,TIME(repair.Modify_Date) AS xTime,repair.IsStatus,repair.RefDocNo
    FROM repair
    INNER JOIN department ON repair.DepCode = department.DepCode
    INNER JOIN site ON department.HptCode = site.HptCode
    INNER JOIN users ON repair.Modify_Code = users.ID
  ...
  function getImport($conn, $DATA)
  {
    ...
    for ($i = 0; $i < $max; $i++) {
      ...
      if ($chkUpdate == 0) {
        if ($Sel == 1) {
          $Sql = " INSERT INTO repair_detail(DocNo, ItemCode, UnitCode, Qty, Weight, IsCancel)
  ...
  function CancelBill($conn, $DATA)
  {
    ...
    // mysqli_query($conn,$Sql);
    $Sql = "UPDATE repair SET IsStatus = 2  WHERE DocNo = '$DocNo'";
    $meQuery = mysqli_query($conn, $Sql);
  }
