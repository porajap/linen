...
VALUES
('$DocNo',DATE(NOW()),$deptCode,'','Factory IN',$userid,DATE(NOW()))";
    mysqli_query($conn, $Sql);
...
function ShowDocument($conn, $DATA)
{
  ...
  // mysqli_query($conn,$Sql);
  $Sql = "SELECT site.HptName,department.DepName,factory_in.DocNo,factory_in.DocDate,factory_in.RefDocNo,factory_in.Total,users.FName,TIME(factory_in.Modify_Date) AS xTime,factory_in.IsStatus
FROM factory_in
INNER JOIN department ON factory_in.DepCode = department.DepCode
INNER JOIN site ON department.HptCode = site.HptCode
INNER JOIN users ON factory_in.Modify_Code = users.ID ";
  if ($selecta == 0) {
    $Sql .= "WHERE factory_in.DepCode = $deptCode AND factory_in.DocNo LIKE '%$DocNo%'";
  }
  $Sql .= "ORDER BY factory_in.DocNo DESC LIMIT 500";
  $meQuery = mysqli_query($conn, $Sql);
  while ($Result = mysqli_fetch_assoc($meQuery)) {
    $return[$count]['HptName']   = $Result['HptName'];
    $return[$count]['DepName']   = $Result['DepName'];
...
function SelectDocument($conn, $DATA)
{
  ...
  $DocNo = $DATA["xdocno"];
  $Datepicker = $DATA["Datepicker"];
  $Sql = "SELECT   site.HptName,department.DepName,factory_in.DocNo,factory_in.DocDate,factory_in.Total,users.FName,TIME(factory_in.Modify_Date) AS xTime,factory_in.IsStatus,factory_in.RefDocNo
FROM factory_in
INNER JOIN department ON factory_in.DepCode = department.DepCode
INNER JOIN site ON department.HptCode = site.HptCode
INNER JOIN users ON factory_in.Modify_Code = users.ID
WHERE factory_in.DocNo = '$DocNo'";
  $meQuery = mysqli_query($conn, $Sql);
  while ($Result = mysqli_fetch_assoc($meQuery)) {
    $return[$count]['HptName']   = $Result['HptName'];
    $return[$count]['DepName']   = $Result['DepName'];
...
function getImport($conn, $DATA)
{
  ...
  for ($i = 0; $i < $max; $i++) {
    ...
    $Sql = "SELECT COUNT(*) as Cnt
		  ...
		  INNER JOIN item  ON factory_in_detail.ItemCode = item.ItemCode
		  INNER JOIN factory_in ON factory_in.DocNo = factory_in_detail.DocNo
		  WHERE factory_in.DocNo = '$DocNo'
    ...
    if ($chkUpdate == 0) {
      if ($Sel == 1) {
        $Sql = "INSERT INTO factory_in_detail(DocNo, ItemCode, UnitCode, Qty, Weight, IsCancel)
            VALUES('$DocNo', '$ItemCode', $iunit2, $iqty2, $iweight, 0) ";
        mysqli_query($conn, $Sql);
      } else {
        $Sql = "INSERT INTO factory_in_detail_sub(DocNo, ItemCode, UsageCode)
            VALUES('$DocNo', '$ItemCode', '$UsageCode') ";
        mysqli_query($conn,$Sql);

        $Sql = "UPDATE item_stock SET IsStatus = 4 WHERE UsageCode = '$UsageCode'";
        mysqli_query($conn, $Sql);
      }
    } else {
      if ($Sel == 1) {
        $Sql = "UPDATE factory_in_detail
  ...
  if ($Sel == 2) {
    $n = 0;
    $Sql = "SELECT COUNT(*) AS Qty FROM factory_in_detail_sub WHERE DocNo = '$DocNo' AND ItemCode = '$ItemCode'";
    $meQuery = mysqli_query($conn, $Sql);
    while ($Result = mysqli_fetch_assoc($meQuery)) {
      $Qty[$n] = $Result['Qty'];
      $n++;
    ...
    for ($i = 0; $i < $n; $i++) {
      ...
      // mysqli_query($conn,$Sqlx);
      if ($chkUpdate == 0) {
        $Sql = "INSERT INTO factory_in_detail
              (DocNo,ItemCode,UnitCode,Qty,Weight,IsCancel)
              VALUES
              ('$DocNo','$ItemCode',$iunit2,$xQty,0,0)";
      } else {
        $Sql = "UPDATE factory_in_detail SET Qty = $xQty WHERE DocNo = '$DocNo' AND ItemCode = '$ItemCode'";
      }
      mysqli_query($conn, $Sql);
    }
  }
...
function UpdateDetailQty($conn, $DATA)
{
  ...
  $Sql = "UPDATE factory_in_detail
	...
	WHERE factory_in_detail.Id = $RowID";
  mysqli_query($conn, $Sql);
  ShowDetail($conn, $DATA);
}
...
function DeleteItem($conn, $DATA){
  ...
  for ($i = 0; $i < $n; $i++) {
    ...
    $Sql = "UPDATE item_stock SET IsStatus = 3 WHERE UsageCode = '$xUsageCode'";
    mysqli_query($conn, $Sql);
  }

  $Sql = "DELETE FROM factory_in_detail_sub
	WHERE DocNo = '$DocNo' AND ItemCode = '$ItemCode'";
  mysqli_query($conn, $Sql);

  $Sql = "DELETE FROM factory_in_detail
	WHERE factory_in_detail.Id = $RowID";
  mysqli_query($conn, $Sql);
  ShowDetail($conn, $DATA);
}
...
function SaveBill($conn, $DATA){
  ...
  $DocNo2 = $DATA["xdocno2"];
  $isStatus = $DATA["isStatus"];
  $Sql = "UPDATE factory_in SET IsStatus = $isStatus WHERE factory_in.DocNo = '$DocNo'";
  mysqli_query($conn, $Sql);
...
function UpdateRefDocNo($conn, $DATA){
  ...
  // mysqli_query($conn,$Sqlx);
  $Sql = "UPDATE factory_in SET RefDocNo = '$RefDocNo' WHERE DocNo = '$DocNo'";
  mysqli_query($conn, $Sql);
  $n = 0;
  $Sql = "SELECT
  ...
  WHERE  dirty_detail_sub.DocNo = '$RefDocNo'";
  $meQuery = mysqli_query($conn, $Sql);
  while ($Result = mysqli_fetch_assoc($meQuery)) {
    $ItemCode = $Result['ItemCode'];
    $UsageCode[$n] = $Result['UsageCode'];
...
function ShowDetail($conn, $DATA)
{
  ...
  //==========================================================
  $Sql = "SELECT
  factory_in_detail.Id,
  factory_in_detail.DocNo,
  factory_in_detail.ItemCode,
  item.ItemName,
  item.UnitCode AS UnitCode1,
  item_unit.UnitName,
  factory_in_detail.UnitCode AS UnitCode2,
  factory_in_detail.Weight,
  factory_in_detail.Qty
FROM item
INNER JOIN item_category ON item.CategoryCode = item_category.CategoryCode
INNER JOIN factory_in_detail ON factory_in_detail.ItemCode = item.ItemCode
INNER JOIN item_unit ON factory_in_detail.UnitCode = item_unit.UnitCode
  WHERE factory_in_detail.DocNo = '$DocNo'
  ORDER BY factory_in_detail.Id DESC";
  $meQuery = mysqli_query($conn, $Sql);
  while ($Result = mysqli_fetch_assoc($meQuery)) {
...
$return[$m5][$count] = $count2;
  ...
  $Sql = "UPDATE factory_in SET Total = $Total WHERE DocNo = '$DocNo'";
  mysqli_query($conn, $Sql);
...
function CancelBill($conn, $DATA)
{
  ...
  // mysqli_query($conn,$Sql);
  $Sql = "UPDATE factory_in SET IsStatus = 2  WHERE DocNo = '$DocNo'";
  mysqli_query($conn, $Sql);
}
