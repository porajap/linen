...
function ShowDetail($conn, $DATA)
{
  ...
  WHERE billcustomer_detail.DocNo = '$DocNo' AND HptCode = '$HptCode'
      ...
      while ($P_Result1 = mysqli_fetch_assoc($P_Query1)) {
        ...
        $Multiply1  = $P_Result1['Multiply'];
         ...
         for ($i = 0; $i < $Qty; $i++) {
          $total = $total + $cal;
                    // ============================================================
                    $insert = "UPDATE billcustomer_detail SET  Price = $cal WHERE ItemCode = '$ItemCode ' AND DocNo = '$DocNo'";
  ...
  if( $UniCode2 >0){
    $PriceUnit = "SELECT item_multiple_unit.PriceUnit FROM item_multiple_unit 
      ...
      while ($PUResult = mysqli_fetch_assoc($PUQuery)) {
        ...
        $insert = "UPDATE billcustomer_detail SET Total = $CusPrice  WHERE ItemCode = '$ItemCode' AND DocNo = '$DocNo'";
        $INQuery = mysqli_query($conn, $insert);
        // ============================================================
      }
  }else if ($weight==0 ){
