...
class PDF extends FPDF
{
  function Header()
  {
    ...
    if($this->page==1){
      ...
      $this->Cell(80);
      $this->Cell(30,10,iconv("UTF-8","TIS-620","Customer Bill"),0,0,'C');
      // Line break
      $this->Ln(7);
    }else{
      $this->Ln(15);
...
$Sql = "SELECT
        billcustomer.DocNo,
        DATE_FORMAT(billcustomer.DocDate,'%d-%m-%Y') AS DocDate,
        site.HptName,
        department.DepName,
        employee.FirstName,
        employee.LastName,
        ...
        TIME(billcustomer.Modify_Date) AS xTime
        FROM
        billcustomer
        INNER JOIN billcustomer_detail ON billcustomer.DocNo = billcustomer_detail.DocNo
        INNER JOIN site ON billcustomer.HptCode = site.HptCode
...
$query = "SELECT
          ...
          FROM billcustomer_detail
					INNER JOIN item_unit ON billcustomer_detail.UnitCode2 = item_unit.UnitCode
...
$width = array(15,35,55,20,20,20,20);
