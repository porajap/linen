...
item.weight,
CASE
WHEN clean.Total > dirty.Total THEN COALESCE(ROUND( (-1*((clean.Total - dirty.Total ) / clean.Total) * 100), 2),'0'  )
WHEN dirty.Total > clean.Total  THEN COALESCE(ROUND( (-1*((dirty.Total - clean.Total ) / dirty.Total) * 100), 2),'0'  )
...
$Sql1 = "SELECT
          ...
          COALESCE(sum(item.weight *rewash_detail.qty ),'0') AS TOTAL 
          FROM rewash
