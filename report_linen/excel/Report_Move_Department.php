...
$objPHPExcel->getActiveSheet()->setCellValue('A5', 'Report_Move_Department');
...
$objPHPExcel->getActiveSheet()->mergeCells('A6:L6');
  ...
  if($DepCode == 'ALL'){
    ...
    $wheredep = "AND department.DepCode = '$DepCode' AND move_department.DepCodeForm = '$DepCode' ";
  }

  
  $wheredate = "";
  if($chk =="one"){
    $wheredate = "AND DATE(move_department.DocDate) = DATE('$date1')";
  }else if($chk =="between"){
    $wheredate = "AND DATE(move_department.DocDate) BETWEEN '$date1' AND '$date2'";
  }else if("month"){
    $wheredate = "AND MONTH(move_department.DocDate) = '$date1'";
  }
    ...
    $Sqldep = "   SELECT
                    ...
                    move_department.approveDate,
                    move_department.Modify_Date,
                    TIMEDIFF(move_department.Modify_Date,move_department.revealDate) AS dateCal
                  FROM
                    move_department
                    INNER JOIN department ON move_department.DepCodeForm = department.DepCode
                  WHERE
                  move_department.SiteCode = '$HptCode' 
                    $wheredate
  ...
  // Rename worksheet
  $objPHPExcel->getActiveSheet()->setTitle("ย้ายแผนก");
...
$file_name = "Report_Move_Department_xls_" . $date . "_" . $h . "_" . $i . "_" . $s . ")";
//
