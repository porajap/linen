...
$objPHPExcel->getActiveSheet()->setCellValue('A5', 'Report_Edit_Par');
...
$objPHPExcel->getActiveSheet()->mergeCells('A6:L6');
  ...
  if($DepCode == 'ALL'){
    ...
    $wheredep = "AND department.DepCode = '$DepCode' AND request_par.DepCode = '$DepCode' ";
  }

  
  $wheredate = "";
  if($chk =="one"){
    $wheredate = "AND DATE(request_par.DocDate) = DATE('$date1')";
  }else if($chk =="between"){
    $wheredate = "AND DATE(request_par.DocDate) BETWEEN '$date1' AND '$date2'";
  }else if("month"){
    $wheredate = "AND MONTH(request_par.DocDate) = '$date1'";
  }
    ...
    $AorZHeader ++;
    $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$AorZHeader] . $row, 'รายการ');
    $AorZHeader ++;
    $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$AorZHeader] . $row, 'ยอด Par ตั้งต้น');
    $AorZHeader ++;
    $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$AorZHeader] . $row, 'ยอด Par ใหม่');
    ...
    $Sqldep = "   SELECT
                  ...
                  FROM
                    request_par
                    INNER JOIN request_par_detail ON request_par.DocNo = request_par_detail.DocNo
    ...
    while ($Resultdep = mysqli_fetch_assoc($meQueryDep)) {
      ...
      $_ItemName = $Resultdep['ItemName'];
      $_ParQty = $Resultdep['ParQty'];
      $_Qty = $Resultdep['Qty'];
      ...
      $detail1 = $date_cell1[$AorZDetail] . $row;
        ...
        $AorZDetail ++;
        $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$AorZDetail] . $row, $_ItemName );
        $AorZDetail ++;
        $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$AorZDetail] . $row, $_ParQty );
        $AorZDetail ++;
        $objPHPExcel->getActiveSheet()->setCellValue($date_cell1[$AorZDetail] . $row, $_Qty );
  ...
  // Rename worksheet
  $objPHPExcel->getActiveSheet()->setTitle("แก้ไขยอด Par");
...
$file_name = "Report_Edit_Par_xls_" . $date . "_" . $h . "_" . $i . "_" . $s . ")";
//
