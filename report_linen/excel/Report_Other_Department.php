...
$objPHPExcel->getActiveSheet()->setCellValue('A5', 'Report_Other_Department');
...
$objPHPExcel->getActiveSheet()->mergeCells('A6:L6');
  ...
  if($DepCode == 'ALL'){
    ...
    $wheredep = "AND department.DepCode = '$DepCode' AND other_department.DepCode = '$DepCode' ";
  }

  
  $wheredate = "";
  if($chk =="one"){
    $wheredate = "AND DATE(other_department.DocDate) = DATE('$date1')";
  }else if($chk =="between"){
    $wheredate = "AND DATE(other_department.DocDate) BETWEEN '$date1' AND '$date2'";
  }else if("month"){
    $wheredate = "AND MONTH(other_department.DocDate) = '$date1'";
  }
    ...
    $Sqldep = "   SELECT
                    ...
                    other_department.approveDate,
                    other_department.Modify_Date,
                    TIMEDIFF(other_department.Modify_Date,other_department.revealDate) AS dateCal
                  FROM
                  other_department
                    INNER JOIN department ON other_department.DepCode = department.DepCode
                  WHERE
                    other_department.SiteCode = '$HptCode' 
                    $wheredate
  ...
  // Rename worksheet
  $objPHPExcel->getActiveSheet()->setTitle("ร้องขออื่นๆ");
...
$file_name = "Report_Other_Department_xls_" . $date . "_" . $h . "_" . $i . "_" . $s . ")";
//
