...
// โรงซักกับโรงพยาบาลภาษาไทย - อังกฤษ
if ($language == 'th') {
  $HptName = 'HptNameTH';
  $FacName = 'FacNameTH';
} else {
...
foreach ($GroupCode as $keyGroup => $GroupCodeloop) {
  $status_group == 0;
  // -----------------------------------------------------------------------------------
  $objPHPExcel->setActiveSheetIndex($keyGroup)
  ...
  $query = "SELECT
                ...
                FROM
                  shelfcount
                  INNER JOIN ( SELECT DepCode, DepName, HptCode,GroupCode FROM department ) AS department ON shelfcount.DepCode = department.DepCode
                  INNER JOIN ( SELECT GroupCode, GroupName FROM grouphpt ) AS grouphpt ON department.GroupCode = grouphpt.GroupCode
                  WHERE grouphpt.GroupCode = '$GroupCodeloop'
  ...
  foreach ($DepCode as $keyDep => $DepCodeloop) {
    ...
    $data = " SELECT
              ...
              FROM
                report_sc
              INNER JOIN (SELECT DocNo , isStatus , SiteCode , complete_date FROM shelfcount) AS shelfcount  ON shelfcount.DocNo = report_sc.DocNo
  ...
  $data .=       " )  AND shelfcount.isStatus <> 9 
                      ...
                      AND shelfcount.isStatus <> 0
                      $categorywhere
                      AND grouphpt.GroupCode =  '$GroupCodeloop'
                      AND department.HptCode = '$HptCode' 
  ...
  // Rename worksheet
  $objPHPExcel->getActiveSheet()->setTitle($GroupName[$keyGroup]);
