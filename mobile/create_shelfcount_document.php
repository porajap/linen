...
$chk = false;
    ...
    $count = false;
		...
		// =======================================================================================
    $Sql = "SELECT CONCAT('SC',lpad($HptCode, 3, 0),'/',SUBSTRING(YEAR(DATE(NOW())),3,4),LPAD(MONTH(DATE(NOW())),2,0),'-',
    LPAD( (COALESCE(MAX(CONVERT(SUBSTRING(DocNo,12,5),UNSIGNED INTEGER)),0)+1) ,5,0)) AS DocNo,DATE(NOW()) AS DocDate,
    CURRENT_TIME() AS RecNow
    FROM shelfcount
    WHERE DocNo Like CONCAT('SC',lpad($HptCode, 3, 0),'/',SUBSTRING(YEAR(DATE(NOW())),3,4),LPAD(MONTH(DATE(NOW())),2,0),'%')
